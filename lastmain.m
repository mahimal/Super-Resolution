 function HR2= lastmain(t,I,V_gvf,R,y4,L,L1,Z,fx,fy)
...
HR1=real(-sigma1.*y4-sigma4.*L+c.*sigma4.*L1+conv2(Z,C,'same'));%.*sqrt(mtimes(V_gvf,transpose(R)));%+sigma_4.*X;%conv2(Z,C,'same');%-sigma1.*y4;%+%-sigma4.*y_2.*sign(imgaussfilt(Img)).*sqrt(mtimes(V_gvf,transpose(R)))+sigma_4.*X;
HR1=HR1(:);
HR2=HR1;
 end